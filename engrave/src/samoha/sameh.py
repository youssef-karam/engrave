...
"""
version = '11'
fontfile = "/home/youssef/catkin_ws/src/caligraphy_robot/engrave/cxf-fonts/FONTS/simplex.yk"
#fontfile = "/home/youssef/catkin_ws/src/caligraphy_robot/engrave/cxf-fonts/cyrillic_ii.cxf"
...
def parse(file):
    ...
    for text in file:
        ...
        if arc_cmd:
            ...
            coords = [float(n) for n in coords.split(',')]
            try:
                xcenter, ycenter, radius, start_angle, end_angle, direction = coords
            ...
            if direction == 1:   #Clockwise
                ...
                segs = 20-(int((end_angle - start_angle) / 20) + 1)
                angleincr = (360-(end_angle - start_angle))/segs
                xstart = cos(start_angle * pi/180) * radius + xcenter
                ystart = sin(start_angle * pi/180) * radius + ycenter
                angle = start_angle
                for i in range(segs):
                    if ((angle-angleincr)<0): #or equal
                        angle = 360+(angle-angleincr)
                    else:
                        angle -= angleincr
...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.ToClipboard = Button(self.EntryFrame, text='To GRBL', command=self.toGrbl)
...
#=======================================================================
    def toGrbl(self):
        f = open("/home/youssef/catkin_ws/src/caligraphy_robot/engrave/grbl.gcode","w+")
        for line in self.gcode:
            #print(type(line))
            f.write(line+'\n')
        pub_gcode.publish("sent")
        f.close()
...
#=======================================================================
    def DoIt(self):
        ...
        #self.gcode.append('( by TIKO AND KARAMELA - 2022 )')
...
#                    self.gcode.append("(%f,%f to %f,%f)" %(stroke.xstart,stroke.ystart,stroke.xend,stroke.yend ))
                    ...
                    if (dist > 0.001) or first_stroke:
                        ...
                        #pub_gcode.publish('G0 Z0')
                        self.gcode.append('G0 X%.4f Y%.4f' %(x1*scale,y1*scale))
                        #pub_gcode.publish('G0 X%.4f Y%.4f' %(x1*scale,y1*scale))
                        self.gcode.append("G0 Z3")
                        #pub_gcode.publish('G1 z10')
                    ...
                    if self.FlipVar.get() == 1:
                        y2 = -y2
                    self.gcode.append('G1 X%.4f Y%.4f' %(x2*scale,y2*scale))
                    #pub_gcode.publish('G1 X%.4f Y%.4f' %(x2*scale,y2*scale))
        ...
        self.gcode.append( 'G0 Z0')     # final engraver up
        #pub_gcode.publish( 'G0 Z0')

        # finish up with icing
        self.gcode.append(self.PostambleVar.get())
        #pub_gcode.publish(str(self.PostambleVar.get()))
        
        
       
        

rospy.init_node('engrave')
pub_gcode = rospy.Publisher('gcode_topic', String, queue_size=10)
app = Application()
app.master.title("engrave-"+version+".py by TIKO AND KARAMELA ")
app.mainloop()
